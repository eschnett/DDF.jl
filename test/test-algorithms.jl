...
@testset "Circumcentre D=$D N=$N" for D = 1:Dmax, N = 1:D+1
    T = Rational{Int256}
    for N = 1:D+1
        xs = SVector{N}(rand(Form{D,1,T}) for n = 1:N)
        cc = circumcentre(xs)
        cc::Form{D,1,T}
        # Check that circumcentre has same distance from all vertices
        r2 = abs2(xs[1] - cc)
        @test all(==(r2), abs2(xs[n] - cc) for n = 1:N)
...
@testset "Regular simplex D=$D" for D = 1:Dmax
    N = D + 1
    T = Float64
